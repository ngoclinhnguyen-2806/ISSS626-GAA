...
| `sig` | Alpha level for significance threshold (e.g., `0.05`). |
...
The LISA clusters map synthesizes these findings into actionable categories. 
- High-High clusters (dark red, 99 hexagons, 4.6%) represent statistically significant hotspots in residential estates where high boarding activity is surrounded by similarly high values—priority zones for bus capacity planning. - Low-High outliers (light blue, 77 hexagons, 3.6%) indicate locations with lower boarding surrounded by high-activity neighbors, possibly representing parks, industrial zones, or transit-dependent areas worth investigating. 
- The overwhelming grey coverage (91.8% not significant) confirms that meaningful spatial clustering is highly localized. 
- The absence of Low-Low coldspots suggests relatively uniform baseline coverage, while missing High-Low patterns indicate demand is geographically coherent without isolated anomalies.
...
plot_moran_ii_pvalue(lisa_wda, "Weekday_Afternoon_Peak")
plot_moran_quadrant(lisa_wda, "Weekday_Afternoon_Peak")
```
...
The LISA clusters map identifies: 
- 84 High-High clusters (3.9%), falling between morning peak (99) and afternoon peak (51), representing mixed-use zones serving weekend activities. 
- Low-High outliers number 66 (3.1%), the lowest of three periods, suggesting more spatially coherent leisure travel patterns. 
