...
calculate_local_morans <- function(data, column_name) {
  ...
  # Calculate local moran's I using spdep directly for more control
  ...
  data %>%
    mutate(
      Ii = lisa[, 1],           # Local Moran's I statistic
      E_Ii = lisa[, 2],         # Expectation
      Var_Ii = lisa[, 3],       # Variance
      Z_Ii = lisa[, 4],         # Z-score
      p_value = lisa[, 5],      # P-value from permutation
      # Classification based on significance and quadrant
...
```
#| code-summary: Helper function to convert nb and wt to listw
nb2listw <- function(nb, wt, style = "W", zero.policy = TRUE) {
  # Convert sfdep nb/wt structure to spdep listw structure
  spdep::nb2listw(nb, glist = wt, style = style, zero.policy = zero.policy)
}
```
