...
**plot_moran_ii** returns Local Moran’s I statistic on the map

**plot_moran_ii_pvalue** returns Local Moran’s I statistics' p-value on the map

**plot_moran_quadrant** returns High-High, Low-Low, Low-High, High-Low clusters on the map
...
```{r}
#| code-summary: Plot moran quadrant
...
```{r}
#| code-summary: Compare Local Moran's I and Gi* for Weekday Morning Peak

# Create comparison dataset
morning_comparison <- morning_moran %>%
  st_drop_geometry() %>%
  select(HEX_ID, Weekday_Morning_Peak, moran_class, p_value) %>%
  left_join(
    morning_gi %>% st_drop_geometry() %>% 
...
cat("Total hexagons analyzed:", nrow(morning_comparison), "\n")
cat("Significant in Moran's I:", sum(morning_moran$p_value < 0.05), "\n")
cat("Significant in Gi*:", sum(morning_gi$p_value_gi < 0.05), "\n")
cat("Significant in BOTH methods:", sum(morning_comparison$both_significant), "\n\n")

cat("Hotspot agreement (High-High = Hot Spot):", 
    sum(morning_comparison$hotspot_agreement, na.rm = TRUE), "\n")
cat("Coldspot agreement (Low-Low = Cold Spot):", 
    sum(morning_comparison$coldspot_agreement, na.rm = TRUE), "\n\n")

# Cross-tabulation
cat("Cross-tabulation of classifications:\n")
comparison_table <- table(
  Moran_I = morning_comparison$moran_class,
  Gi_star = morning_comparison$gi_class
)
...
cat("Morning-only hotspots:", 
    sum(temporal_comparison$morning_only), "\n")
cat("Afternoon-only hotspots:", 
    sum(temporal_comparison$afternoon_only), "\n")
cat("Weekend-only hotspots:", 
