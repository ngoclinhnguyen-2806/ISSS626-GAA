...
Singapore's public housing market, dominated by Housing & Development Board (HDB) flats, accommodates over 80% of the resident population. Resale flat prices exhibit substantial geographic variation—a 4-room flat in Bukit Timah commands premiums exceeding 50% over identical units in Woodlands. Traditional hedonic pricing models using ordinary least squares (OLS) regression assume price determinants operate uniformly across space, yet Singapore's diverse urban geography—ranging from CBD-proximate mature estates to peripheral new towns—suggests pricing mechanisms vary locally.

Geographically Weighted Regression (GWR) addresses this limitation by allowing relationships between price and predictors to vary spatially, capturing neighborhood-specific market dynamics. Understanding these spatial heterogeneities informs targeted housing policies, assists buyers in location-specific valuation, and advances urban economics research on residential property markets.

### 1.2 Research Objectives

This study aims to:

-   Calibrate a GWR model to identify and evaluate factors influencing HDB 4-room resale prices (January–September 2025)

-   Quantify spatial non-stationarity in key price determinants: structural attributes (floor area, storey, lease), proximity to amenities (CBD, parks, eldercare), and neighborhood accessibility (bus stops)

-   Compare GWR performance against OLS baseline to demonstrate benefits of spatial modeling

-   Map local coefficient variations to reveal Singapore's housing submarkets and their distinct pricing mechanisms
...
The distributions reveal typical 4-room HDB characteristics: standardized floor areas clustered at 93 sqm, right-skewed resale prices (median $630,000), and concentration at lower storey levels reflecting Singapore's predominantly mid-rise public housing. The bimodal remaining lease distribution indicates a mix of older and newer estates, with most flats having 40-95 years remaining.
...
These facility count distributions show Singapore's well-developed public infrastructure. Preschools (mean 5.5) and childcare centers (mean 4.7) demonstrate strong early childhood support. The extremely low kindergarten count (mean 0.9) reflects Singapore's integration of kindergartens into preschools. Bus stops average 8 per buffer zone, indicating excellent public transport accessibility crucial for car-lite HDB estates.Retry
...
Proximity statistics reveal spatial patterns of Singapore's urban planning. Most amenities are highly accessible: hawker centers (median 647m), parks (661m), and supermarkets (278m) are within walkable distances. The CBD's greater distance (median 12.9km) reflects HDB estates' suburban locations. Eldercare facilities show wider dispersion (SD 634m), possibly indicating uneven distribution across neighborhoods.
...
Transaction volumes show typical seasonal patterns with a notable spike in January 2025 and subsequent decline. Resale prices demonstrate steady upward momentum throughout 2024-2025, with median prices rising from $620k to $640k. The widening gap between mean and median prices suggests increasing price dispersion, potentially driven by premium transactions in mature estates or renovated units.
...
Central Area's premium pricing reflects Singapore's stark locational hierarchy, where proximity to employment and amenities justifies paying double compared to peripheral towns. The price compression in mature estates suggests gentrification limits, while high transaction volumes in new towns like Sengkang and Tampines indicate these areas are maturing into self-sufficient regional hubs, potentially reshaping Singapore's traditionally CBD-centric urban structure.
...
Model A's dominance (66.5% of transactions) reflects HDB's mass standardization, but DBSS ($880k) and Type S1 ($1.37M) flats command significant premiums. This growing price gap suggests buyers increasingly pay extra to escape standard designs, indicating Singapore's public housing market is splitting into tiers. This trend may challenge HDB's goal of maintaining social equality as income differences become more visible in housing choices.
...
Floor area shows minimal price impact due to standardization, while remaining lease and storey level demonstrate clear positive relationships with price. CBD distance shows expected negative correlation—properties closer to the CBD command higher prices due to work commute convenience and urban amenities. This strong distance-decay effect suggests CBD accessibility remains a primary price driver, warranting closer examination of how centrality shapes Singapore's housing market.
...
GWR substantially outperforms OLS, with R² improving from 0.77 to 0.97—explaining 97% of price variance compared to OLS's 77%. The model uses 173 nearest neighbors, allowing coefficients to vary locally. Spatial autocorrelation in residuals dropped 94% (Moran's I: 0.81 → 0.05), confirming GWR successfully accounts for spatial non-stationarity.

Local R² ranges from 0.65 to 0.99 (median = 0.96), showing the model fits well across most locations. Lower R² in peripheral areas suggests evolving market dynamics in newer developments.

### 5.2 Local R² Statistics

```{r}
#| label: map-local-r2
#| code-summary: Local R² - Model Fit Quality
#| fig-width: 10
#| fig-height: 8
...
tm_shape(hdb_gwr) +
  tm_dots(col = "local_r2",
          ...
          title = "Local R²") +
  tm_layout(
    main.title = "Map 6: Local R² - Model Fit Quality",
    main.title.size = 1.1,
    legend.outside = TRUE,
    frame = FALSE
  ) +
...
The model performs best (R² \> 0.96) in mature central estates with stable pricing patterns. Lower R² (0.65–0.85) appears in peripheral developments, reflecting greater price unpredictability from ongoing infrastructure changes. The consistently high median R² (0.96) demonstrates GWR effectively captures localized market dynamics across Singapore.

### 5.3 Distance to CBD Effect

```{r}
#| label: map-coef-cbd
#| code-summary: Local Coefficient - Distance to CBD
#| fig-width: 10
#| fig-height: 8
...
tm_shape(singapore_boundary) +
  ...
  tm_fill(col = "grey95") +
tm_shape(hdb_gwr) +
  tm_dots(col = "coef_cbd",
          palette = "-RdBu",
          style = "jenks",
          n = 7,
          size = 0.06,
          alpha = 0.7,
          title = "CBD Coefficient",
          midpoint = 0) +
  tm_layout(
    main.title = "Map 7: Local Coefficient - Distance to CBD",
    main.title.size = 1.1,
    legend.outside = TRUE,
    frame = FALSE
  ) +
...
CBD proximity strongly affects prices in central and eastern regions (negative coefficients), where each kilometer further reduces prices by \$200+. Peripheral northern and western towns show near-zero effects, indicating CBD distance matters little—residents prioritize regional centers instead. This reveals Singapore has distinct housing submarkets with different valuation priorities.

### 5.4 Storey Level Premium

```{r}
#| label: map-coef-storey
#| code-summary: Local Coefficient - Storey Level
#| fig-width: 10
#| fig-height: 8
...
tm_shape(singapore_boundary) +
  ...
  tm_fill(col = "grey95") +
tm_shape(hdb_gwr) +
  tm_dots(col = "coef_storey",
          palette = "RdYlGn",
          style = "jenks",
          n = 7,
          size = 0.06,
          alpha = 0.7,
          title = "Storey Coefficient") +
  tm_layout(
    main.title = "Map 8: Local Coefficient - Storey Level",
    main.title.size = 1.1,
    legend.outside = TRUE,
    frame = FALSE
  ) +
...
Floor level premiums range from \$1,987 to \$8,575 per storey. Higher premiums appear in high-density central areas and coastal towns where views and air quality command significant value. Peripheral estates show moderate uniform premiums (\$3,500–4,500), indicating standard height-based pricing without exceptional view premiums.

### 5.5 Remaining Lease Effect

```{r}
#| label: map-coef-lease
#| code-summary: Local Coefficient - Remaining Lease
#| fig-width: 10
#| fig-height: 8
...
tm_shape(singapore_boundary) +
  ...
  tm_fill(col = "grey95") +
tm_shape(hdb_gwr) +
  tm_dots(col = "coef_lease",
          palette = "PuOr",
          style = "jenks",
          n = 7,
          size = 0.06,
          alpha = 0.7,
          title = "Lease Coefficient",
          midpoint = 5183) +
  tm_layout(
    main.title = "Map 9: Local Coefficient - Remaining Lease",
    main.title.size = 1.1,
    legend.outside = TRUE,
    frame = FALSE
  ) +
...
Lease tenure effects vary widely (-\$35,909 to +\$18,729 per year). Mature estates with older stock show high lease sensitivity (β \> \$8,000/year), where lease decay significantly impacts prices. Newer towns show lower coefficients as most units retain long leases, making tenure less critical.

### 5.6 OLS vs GWR Residuals

```{r}
#| label: map-residuals-comparison
#| code-summary: Compare OLS and GWR residuals
#| fig-width: 14
#| fig-height: 8

map_ols <- tm_shape(singapore_boundary) +
  tm_borders(col = "grey50", lwd = 1) +
  tm_fill(col = "grey95") +
tm_shape(hdb_with_residuals) +
  tm_dots(col = "ols_residuals",
          ...
          title = "OLS Residuals",
          midpoint = 0) +
  tm_layout(main.title = "OLS Residuals", frame = FALSE)

map_gwr <- tm_shape(singapore_boundary) +
  tm_borders(col = "grey50", lwd = 1) +
  tm_fill(col = "grey95") +
tm_shape(hdb_gwr) +
  tm_dots(col = "gwr_residuals",
          ...
          title = "GWR Residuals",
          midpoint = 0) +
  tm_layout(main.title = "GWR Residuals", frame = FALSE)

tmap_arrange(map_ols, map_gwr, ncol = 2)
```

OLS residuals show strong spatial clustering (Moran's I = 0.81): under-prediction in central mature estates (blue) and over-prediction in peripheral towns (red). GWR residuals are nearly random (Moran's I = 0.05), with clustering reduced 94%. This visual comparison proves GWR successfully captures geographic pricing differences that OLS's uniform coefficients cannot represent.

### 5.7 Key Findings

**Three distinct housing submarkets identified:**

-   CBD-centric mature estates (central/eastern): CBD proximity dominates, with strong negative distance effects (-\$200+ per km).

-   Regional hubs (Tampines, Jurong): CBD distance irrelevant; storey and lease premiums drive prices.

-   Peripheral towns (northern/western): Moderate uniform effects across all variables; lower model fit suggests transitional markets.

**Implications:**

-   For buyers: Location matters differently by area. In mature estates, proximity trumps unit features; in new towns, floor area and storey matter more.

-   For policymakers: Cooling measures and transport investments have spatially uneven impacts. Lease decay management requires geographic targeting.

-   Model limitations: Small residual autocorrelation persists, suggesting unmeasured factors (school reputation, crime) remain unaccounted.

## 6. Limitations & Conclusions

### 6.1 Conclusions

This study successfully applied GWR to model HDB 4-room resale prices, achieving 97% explanatory power—a substantial improvement over OLS (77%). Spatial analysis confirmed three distinct submarkets: CBD-centric mature estates, self-sufficient regional hubs, and transitional peripheral towns, each exhibiting unique pricing mechanisms.

GWR's local coefficients revealed that CBD proximity, storey premiums, and lease sensitivity vary dramatically across Singapore's geography, validating the spatial non-stationarity hypothesis. The 94% reduction in residual autocorrelation demonstrates GWR effectively captures spatial heterogeneity that global models cannot represent.

These findings inform spatially-targeted housing policies and provide buyers with location-specific valuation insights, advancing understanding of Singapore's complex residential property market.

### 6.2 Study Limitations

-   Variable selection: Excluded MRT proximity due to data issues. Missing variables (school reputation, crime rates, ethnic composition) may explain residual spatial autocorrelation (Moran's I = 0.05, p \< 0.001).

-   Methodological considerations: Fixed bandwidth (173 neighbors) may over-smooth heterogeneous areas. Multicollinearity among early childhood facilities (VIF \> 10) required variable exclusion, potentially omitting relevant amenity effects.
