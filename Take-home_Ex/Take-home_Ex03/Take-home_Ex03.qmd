...
```{r}
#| code-summary: Prepare dataset for OLS baseline model

# Remove geometry for OLS (non-spatial model)
ols_data <- hdb_model_final %>%
  st_drop_geometry() %>%
  select(
    resale_price,
    floor_area_sqm,
...
cat("✓ OLS dataset prepared\n",
"Observations:", nrow(ols_data), "\n",
"Variables:", ncol(ols_data), "(1 outcome + 9 predictors)\n")
```

```{r}
#| code-summary: Fit ordinary least squares regression model
...
# Fit OLS model
ols_model <- lm(ols_formula, data = ols_data)
...
```{r}
#| code-summary: Calculate OLS model performance statistics
...
# Additional metrics
n_obs <- nrow(ols_data)
n_predictors <- length(coef(ols_model)) - 1  # Exclude intercept
residual_se <- sigma(ols_model)
aic_value <- AIC(ols_model)
bic_value <- BIC(ols_model)

# Create performance summary table
performance_table <- data.frame(
  Metric = c("Observations", "Predictors", "R-squared", "Adjusted R-squared", 
             "F-statistic", "p-value (F-test)", "Residual Std. Error", 
             "AIC", "BIC"),
  Value = c(
    n_obs,
    n_predictors,
    ols_performance$r.squared,
    ols_performance$adj.r.squared,
    ols_performance$statistic,
    ols_performance$p.value,
    residual_se,
    aic_value,
    bic_value
  )
...
knitr::kable(performance_table,
             digits = c(0, 4),
             caption = "Table 12: OLS Model Performance Metrics",
             col.names = c("Metric", "Value"),
             format.args = list(big.mark = ","))

cat("\n",
"Model explains", round(ols_performance$adj.r.squared * 100, 2), 
    "% of variance in resale prices\n")
```

```{r}
#| label: ols-coefficient-interpretation
...
```{r}
#| label: ols-diagnostic-tests
#| code-summary: Statistical tests for OLS assumptions

library(lmtest)

# 1. Breusch-Pagan Test for Heteroscedasticity
bp_test <- bptest(ols_model)
cat("1. BREUSCH-PAGAN TEST (Heteroscedasticity)\n")
cat("   H0: Homoscedasticity (constant variance)\n")
cat("   Test statistic:", round(bp_test$statistic, 4), "\n")
cat("   p-value:", format.pval(bp_test$p.value, eps = 0.001), "\n")
if(bp_test$p.value < 0.05) {
  cat("   → Reject H0: Evidence of heteroscedasticity\n")
  cat("   → Variance of residuals is not constant\n\n")
} else {
  cat("   → Fail to reject H0: No evidence of heteroscedasticity\n\n")
}

# 2. Shapiro-Wilk Test for Normality (on sample if n > 5000)
if(nrow(ols_data) > 5000) {
  sample_residuals <- sample(residuals(ols_model), 5000)
  sw_test <- shapiro.test(sample_residuals)
  cat("2. SHAPIRO-WILK TEST (Normality - sampled)\n")
  cat("   Note: Using random sample of 5,000 residuals\n")
} else {
  sw_test <- shapiro.test(residuals(ols_model))
  cat("2. SHAPIRO-WILK TEST (Normality)\n")
}
cat("   H0: Residuals are normally distributed\n")
cat("   Test statistic:", round(sw_test$statistic, 4), "\n")
cat("   p-value:", format.pval(sw_test$p.value, eps = 0.001), "\n")
if(sw_test$p.value < 0.05) {
  cat("   → Reject H0: Residuals deviate from normality\n")
  cat("   → Large sample size may cause sensitivity\n\n")
} else {
  cat("   → Fail to reject H0: Residuals approximately normal\n\n")
}

# 3. Durbin-Watson Test for Autocorrelation
dw_test <- dwtest(ols_model)
cat("3. DURBIN-WATSON TEST (Autocorrelation)\n")
cat("   H0: No autocorrelation in residuals\n")
cat("   DW statistic:", round(dw_test$statistic, 4), "\n")
cat("   p-value:", format.pval(dw_test$p.value, eps = 0.001), "\n")
if(dw_test$p.value < 0.05) {
  cat("   → Reject H0: Evidence of autocorrelation\n")
  cat("   → Note: This may indicate spatial dependence\n\n")
} else {
  cat("   → Fail to reject H0: No significant autocorrelation\n\n")
}
```
...
```{r}
#| label: ols-residual-spatial-autocorrelation
#| code-summary: Test for spatial autocorrelation in OLS residuals

# Add residuals back to spatial data
hdb_with_residuals <- hdb_model_final %>%
  filter(!is.na(resale_price)) %>%
  slice(1:nrow(ols_data)) %>%  # Match OLS data rows
  mutate(
...
cat("═══════════════════════════════════════════════════════\n")
cat("   SPATIAL AUTOCORRELATION OF OLS RESIDUALS\n")
cat("═══════════════════════════════════════════════════════\n\n")
...
#| code-summary: Analyze distribution of OLS residuals
#| fig-width: 12
#| fig-height: 6

# Histogram and Q-Q plot
p1 <- ggplot(hdb_with_residuals, aes(x = ols_residuals)) +
  geom_histogram(bins = 50, fill = "#440154", alpha = 0.7) +
  geom_vline(xintercept = 0, color = "red", linetype = "dashed", linewidth = 1) +
  labs(title = "Distribution of OLS Residuals",
       x = "Residuals (SGD)", y = "Frequency") +
  scale_x_continuous(labels = scales::comma) +
  theme_minimal()

p2 <- ggplot(hdb_with_residuals, aes(sample = ols_residuals)) +
  stat_qq(alpha = 0.3, color = "#440154") +
...
p1 + p2 +
  plot_annotation(
    title = "Figure 13: OLS Residual Distribution",
...
residual_summary <- data.frame(
  Statistic = c("Mean", "Median", "Std. Deviation", "Min", "Max", 
                "Skewness", "Kurtosis"),
  Value = c(
    mean(hdb_with_residuals$ols_residuals),
    median(hdb_with_residuals$ols_residuals),
    sd(hdb_with_residuals$ols_residuals),
    min(hdb_with_residuals$ols_residuals),
    max(hdb_with_residuals$ols_residuals),
    moments::skewness(hdb_with_residuals$ols_residuals),
    moments::kurtosis(hdb_with_residuals$ols_residuals)
  )
)

knitr::kable(residual_summary,
             digits = 2,
             caption = "Table 13: OLS Residual Summary Statistics",
...
cat("  • R-squared:", round(ols_performance$r.squared, 4), "\n")
cat("  • Adjusted R-squared:", round(ols_performance$adj.r.squared, 4), "\n")
cat("  • RMSE: $", scales::comma(sqrt(mean(residuals(ols_model)^2))), "\n")
cat("  • AIC:", round(aic_value, 2), "\n\n")

cat("SIGNIFICANT PREDICTORS:\n")
sig_count <- sum(ols_results$p.value < 0.05) - 1  # Exclude intercept
cat("  •", sig_count, "out of", n_predictors, "predictors significant (p < 0.05)\n\n")

cat("ASSUMPTION TESTS:\n")
cat("  • Heteroscedasticity:", ifelse(bp_test$p.value < 0.05, "Present ✗", "Absent ✓"), "\n")
cat("  • Normality of residuals:", ifelse(sw_test$p.value < 0.05, "Violated ✗", "Satisfied ✓"), "\n")
cat("  • Spatial autocorrelation:", ifelse(moran_residuals$p.value < 0.05, "Present ✗", "Absent ✓"), "\n\n")

cat("KEY FINDINGS:\n")
cat("  1. Model explains ~", round(ols_performance$adj.r.squared * 100, 1), 
    "% of price variation\n")
cat("  2. CBD proximity is the strongest predictor\n")
cat("  3. OLS residuals show significant spatial autocorrelation\n")
cat("  4. Spatial non-stationarity detected\n\n")

cat("CONCLUSION:\n")
cat("  The OLS model provides a reasonable baseline but violates the\n")
cat("  independence assumption due to spatial autocorrelation in residuals.\n")
cat("  This justifies the use of Geographically Weighted Regression (GWR)\n")
cat("  to account for spatial heterogeneity and improve model performance.\n\n")

cat("✓ OLS baseline model complete\n")
cat("✓ Ready to proceed to Section 4.2 (GWR Model)\n")
cat("═══════════════════════════════════════════════════════\n")
```
