...
Calibrate a GWR model to identify and evaluate factors influencing HDB 4-room resale prices (January–September 2025) Quantify spatial non-stationarity in key price determinants: structural attributes (floor area, storey, lease), proximity to amenities (CBD, parks, eldercare), and neighborhood accessibility (bus stops) Compare GWR performance against OLS baseline to demonstrate benefits of spatial modeling Map local coefficient variations to reveal Singapore's housing submarkets and their distinct pricing mechanisms
