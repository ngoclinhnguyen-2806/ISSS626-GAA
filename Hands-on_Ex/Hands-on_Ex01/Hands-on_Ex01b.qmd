...
The code block below uses st_read() function of sf package to import the `MasterPlan2019SubzoneBoundaryNoSeaKML` shapefile into R as a polygon feature data frame.
...
mpsz <- mpsz %>%
  ...
  select(-Name, -Description) %>% # The dash (-) means "deletion", we want to delete Name and Description columns because we no longer need them.
...
Next, *respopagsextod2024.csv* file is imported into RStudio and save the file into an R dataframe called *popdata*.
...
```{r}
#| fig-width: 12 # default width is 8. Enlarging the width helps accomodating more elements horizontally
tm_shape(mpsz_pop2024) + 
  tm_polygons(
    fill = c("YOUNG", "AGED"),
    fill.legend = 
...
In the code chunk below, We improve the visual representation further by highlighting and labeling the outliers on the choropleth map.
