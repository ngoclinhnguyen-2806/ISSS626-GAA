...
```{r}
pacman::p_load(sf, tidyverse) # pacman is to replace all the library() function call each time we want to import a library
```
...
::: callout-note
Note that the preschool point objects failed to plot on top of the mpsz layer because *preschool* is in WGS 84, while *mpsz* is in SVY21.
:::
...
```{r}
st_crs(mpsz) # This function is just to report the data frame and make no changes
```
...
::: callout-note
We can try plotting listing_sf layer on top of mpsz layer now as both are SVY 21.
:::
