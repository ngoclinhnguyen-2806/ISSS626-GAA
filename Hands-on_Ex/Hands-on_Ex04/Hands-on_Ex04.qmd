...
```{r}
hunan <- left_join(hunan,hunan2012) %>% # Join by County, but no need to explicitly mention because both datasets have this column
  select(1:4, 7, 15)
```
...
Let's take a good look at the neighbour list of area \[1\] by using the code chunk below.
...
Lastly, qtm() of tmap package is used to plot the `lag_sum_GDPPC` and w_sum_gdppc maps next to each other for quick comparison.
